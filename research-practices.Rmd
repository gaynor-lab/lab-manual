...
+ The name should be short but informative.
+ Code for published papers will eventually need to be made public, but you may chose to create a private repository during the early stages of the work.
+ Sensitive data should not be stored on GitHub. Speak with Kaitlyn before uploading any data to github. Take advantage of .gitignore to prevent sensitive data from being uploaded to github.
+ GitHub is not designed for large file storage. If your project involves large files, you may need to find an alternative data management system. Speak with Kaitlyn about this if it is relevant for your project.
...
+ Set up your .gitignore file before your first commit. Files like .DS_Store and .Rhistory should be excluded from the repository.
...
*Adapted from Joey Bernhardt and Mary O'Connor*

Part of the work you are doing in the lab is learning how to think critically, write in your voice, develop your communication skills, and logic through problem solving. While Generative AI and large language models (LLMs) like ChatGPT, Bard, Grammarly, Copilot, etc. can help with this, they are easily used to replace “thinking” in a way that is negative to your professional development at this stage. Yet, LLMs are increasingly integrated into many technologies and can be very convenient tools for a variety of applications that are relevant to your future careers. Taking all of this together, we recognize the usefulness of these tools as well as their limitations, and contextualize that with the environmental impact of increased computing power, equitability and access issues, and general ethical questions around their use (training data sets, intellectual property, etc.).

A note that UBC advocates for the use of Microsoft Copilot, as the data are all stored on Canadian servers, and your inputs are not used to train the model. UBC has an organization version of Copilot that is free for all students and faculty. You can access Copilot at https://portal.office.com/ , and be sure you are logged in with your UBC Microsoft account. **You should not put any data or research ideas into any LLM other than UBC Copilot, as they may be used to train the model, and you have no control over where your intellectual property goes.**

### Coding

Within your work in the Gaynor Lab, LLMs are acceptable to use for coding (e.g. “write a basic script to do X in R”, “what is the command to do X in bash,” “why am I getting this error,”, “what does this code do,” etc.) In fact, you are encouraged to use LLMs as a way to troubleshoot bugs and make your code more efficient. However, you should only be using generative AI or LLMs if you understand the inputs and outputs of what you are asking about, and you must indicate that you have used generative AI in any resulting publication or thesis (this is increasingly a requirement of most journals). If you do use generative AI for coding help, scrutinize each line that comes back so you can learn from the output and also criticize whether each step is actually helpful in what you want to do.

### Writing and idea development

The story is a bit different for writing and organizing your ideas. LLMs really stifle the creative process, and thinking about the best way to structure a paper or paragraph is how you develop new ideas and insights. ChatGPT also reinforces certain worldviews and dominant paradigms, and can constrain your thinking. It also can’t come up with original ideas, which is the whole point of our work. And then later in the writing process, if you use ChatGPT to draft all of your language once the ideas are outlined, it just makes you sound like a robot and takes away your voice. Therefore, LLMs should not be used to draft abstracts, manuscripts, or other technical writing products without consulting Kaitlyn. For writing, you should only use LLMs at the level of a decent spellcheck or thesaurus once you have your draft written, but not to autocomplete sentence or revise writing beyond basic grammar. If you are ever in doubt about whether you are using an LLM in a way that is appropriate for your work here, please discuss with Kaitlyn.

### Background research

LLMs can also be a useful search engine, or a tool for understanding theories and concepts. If you are looking to understand the pros and cons of different analysis approaches, or understand the definition of a foundational concept in ecology, LLMs can get you to answers more quickly and concisely than traditional search engines or wading through journal databases. However, it should always be a first step, not a last step. You should always identify primary literature on the topic as well. Also, while LLMs are good for broad-strokes information, be very wary of asking them for factual information about specific events in history, people, or papers, as it hallucinates things. (I lazily asked ChatGPT to write a biography of me for a talk that I’m giving, and while most things were correct, it said that I graduated from Amherst University as an undergraduate and that I serve on the IUCN Connectivity working group, neither of which are true.) For this reason, you should never use LLMs to identify citations, examples, or experts in the field.


## Developing research projects

### Template for developing research ideas
*Adapted from Joey Bernhardt and Mary O'Connor*

Use this document as an outline, filling in your own text under the headings. I will encourage you to do this at the start of a new project, to help shape its direction and design.

1. **Main Objective:** What is the single main objective of the research? This objective should relate to some kind of general ecological or conservation issue, and be articulated in a single concise statement. 

2. **Testable hypothesis:** Your research should typically be focused around a specific testable
hypothesis or answerable question. Ideally, hypothesis(es) should support some inference about the main objective. See below for detailed guidance on how to develop good hypotheses and predictions.

3. **Methods:** Outline the methods in broad strokes, but specifically in reference to the
evidence you will gather and produce to test predictions (above) and make inference (below). When writing, refer explicitly to the predictions.

+ How will hypotheses be tested (reasoning, and evidence)? General categories include experiments, observations (surveys), meta-analyses, analysis of theoretical models. Will inferences be drawn from more than one type of evidence?
+ How will data be collected and analyzed? What statistical tests and software will you use?
+ What potentially confounding factors or uncertainties are you considering
+ What resources are required? Budget for time (e.g., timeline) and for costs.

4. **Results:** The 2-3 key figures or tables you will produce for your final paper. In the project development stage, sketch these figures, complete with axis labels, units etc. Draw the possible ways the data could look (likely and unlikely) and consider what you would infer if you saw each pattern (would you reject your hypotheses?). Consider a power analysis. Once in the writing phase, distill the results into 2-3 key figures/tables. Identify others as supporting, and clearly relate them in terms of how they are used to advance knowledge (hypotheses, main objective).

5. **Inference and Next Steps:** How exactly do your figures (sketched above), your statistical analysis, and your logic together generate new knowledge? (specifically, by testing hypotheses in service of meeting your main objective?) If in the project development phase, consider different possible versions of the results, and how you might interpret them. Then revisit your questions – are you happy with how your hypotheses are stated, given the possible results? In the writing stage, summarize the evidence and chain of reasoning in a way that speaks directly to the hypotheses and main objective.
...
+ Label sizes (can the axes be easily read). + Units (do all of my axes have units). 
...
+ Save your files as PDF, TIFF or PNGs.

Colour choice is also extremely important when preparing your figures and colour palettes should be chosen with care. Always strive to produce colourblind friendly figures. 
...
If you feel your work may be of interest to the press (i.e., you can condense the finding into a snappy headline that will be of general interest), please discuss with Kaitlyn, and she will get you in touch with UBC's press office.
